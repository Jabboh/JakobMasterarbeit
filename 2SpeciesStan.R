...
#' Create data sets for stan univariate probit model
make_univariate_data <- function(formula, ydata, data) {
  model_matrix <- stats::model.matrix(formula, data)
  list(
    K = NCOL(model_matrix),
    N = NROW(data),
...
cp_data <- make_multivariate_data(rhs_formula, ydata = y_train[,"Cxperpre", drop = FALSE], 
                          data = train)
cp_uni_data <- make_univariate_data(rhs_formula, ydata = y_train$Cxperpre, 
                                data = train)

#' Fit the CP with univariate probit model
cp_uni_fit <- stan(
  file = "univariate_probit.stan",  # Stan program
  data = cp_uni_data,    # named list of data
...
cp_uni_beta <- rstan::extract(cp_uni_fit, pars = "beta")$beta
...
colnames(cp_cp) <- colnames(apcp_cp) <- colnames(cp_uni_beta) <- colnames(cp_data$x)
plot_betas(cp_cp, apcp_cp, "Univariate", "Multivariate", "Culex perexiguus")
plot_betas(cp_cp, cp_uni_beta, "Univariate (Multi)", "Univariate", "Culex perexiguus")

#' Correlations between posterior coefficient estimates
plot(cor(cp_cp), cor(apcp_cp))
abline(0, 1)
plot(cor(cp_uni_beta), cor(apcp_cp))
abline(0, 1)
...
predict_univariate_probit <- function(fit, formula, newdata, n_max = 4000, 
                                      type = "link") {
  model_matrix <- stats::model.matrix(formula, newdata)
  draws_df <- rstan::extract(fit, pars = "beta")$beta
  if (n_max < NROW(draws_df)) {
    i <- sample.int(NROW(draws_df), n_max, replace = FALSE)
    draws_df <- draws_df[i, ]
  }
...
cp_uni_p <- predict_univariate_probit(cp_uni_fit, rhs_formula, train, type = "response")
...
par(mfrow = c(2,2))
plot(rowMeans(cp_p), rowMeans(cp_uni_p), asp = 1, xlim = c(0, 1), ylim = c(0, 1))
abline(0, 1)
plot(rowMeans(cp_p), rowMeans(cp_multi_p), asp = 1, xlim = c(0, 1), ylim = c(0, 1))
abline(0, 1)
plot(rowMeans(cp_uni_p), rowMeans(cp_multi_p), asp = 1, xlim = c(0, 1), ylim = c(0, 1))
abline(0, 1)
plot(rowMeans(cp_uni_p), rowMeans(cp_arm_p), asp = 1, xlim = c(0, 1), ylim = c(0, 1))
abline(0, 1)
par(op)
...
op <- par(no.readonly = TRUE)
par(mfrow = c(2,2))
plot(apply(cp_p, 1, sd), apply(cp_uni_p, 1, sd), asp = 1)
abline(0, 1)
plot(apply(cp_p, 1, sd), apply(cp_multi_p, 1, sd), asp = 1)
abline(0, 1)
plot(apply(cp_uni_p, 1, sd), apply(cp_multi_p, 1, sd), asp = 1)
abline(0, 1)
plot(apply(cp_uni_p, 1, sd), apply(cp_arm_p, 1, sd), asp = 1)
abline(0, 1)
par(op)
