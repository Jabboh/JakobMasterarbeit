...
```{r message=FALSE, results='hide'}
posterior_predict(fit_cp)
```

```{r message=FALSE, results='hide'}
dharm_cp <- createDHARMa(simulatedResponse = t(posterior_predict(fit_cp)),
                         observedResponse = fit_cp$y,
                         fittedPredictedResponse = posterior_predict(fit_cp) %>% apply(2, median),
...
For all Mes and "Indundation Area" no significant problems were detected which supports the 0-hypothesis that we specified the functional relationships of the these covariates correctly. However, for NDVI and NDVI Before, the .75 quantile shows significant deviations from the expected distribution (a flat line). For NDVI, the relationship between residual and NDVI seems somewhat periodic. (Question: This hints at a periodic relationship between NDVI and Response, doesn't it?). 
For NDVI Before, the relationship seems to be somewhat quadratic.
