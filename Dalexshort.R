########Change number of permutation runs B! and nsim of gjam_predict!
...
set.seed(1980)
vi_cp <- model_parts(dal_cp, type = "difference", B = 5)
plot(vi_cp) + 
...
pred_gj <- function(model, newdata)  {
  ...
  newdata <- list(xdata = newdata, nsim = 500)
...
set.seed(1980)
vi_cpgj <- model_parts(dal_cpgj, type = "difference", B = 5) # du musst auf jeden Fall number of samples
...
pred_gjco <- function(model, newdata)  {
  ...
  newdata <- list(xdata = newdata, ydataCond = y_train[,2], nsim = 500)
  #Doing the predictions
  pre <- gjamPredict(output = model, newdata = newdata)
  #return the mean of the y-chains for every observation
  return(pre$sdList$yMu[,1])
...
vi_cpgjco <- model_parts(dal_cpgjco, type = "difference", B = 5) # du musst auf jeden Fall number of samples
...
pred_con <- function(model, newdata)  {
  ...
  newdata$"I(NDVIBEF2000^2)" <- (newdata$NDVIBEF_2000)^2
  #make a newdata list for gjam
  newdata <- list(xdata = newdata, ydataCond = newdata[,match("Anatropre", names(newdata))], nsim = 500)
  #Doing the predictions
  pre <- gjamPredict(output = model, newdata = newdata)
  #return the mean of the y-chains for every observation
  return(pre$sdList$yMu[,1])
...
set.seed(1980)
vi_at <- model_parts(dal_at, type = "difference", B = 5)
#plot it
plot(vi_at) + 
...
#permutation-based variable importance measure
set.seed(1980)
vi_atgj <- model_parts(dal_atgj, type = "difference", B = 5) 
...
pred_atgjco <- function(model, newdata)  {
  ...
  newdata <- list(xdata = newdata, ydataCond = y_train[,1], nsim = 500)
  #Doing the predictions
  pre <- gjamPredict(output = model, newdata = newdata)
  #return the mean of the y-chains for every observation
  return(pre$sdList$yMu[,2])
...
set.seed(1980)
#DOing it for 50 permutations
vi_atgjco <- model_parts(dal_atgjco, type = "difference", B = 5) 
...
pred_atcon <- function(model, newdata)  {
  ...
  newdata$"I(NDVIBEF2000^2)" <- (newdata$NDVIBEF_2000)^2
  #make a newdata list for gjam
  newdata <- list(xdata = newdata, ydataCond = newdata[,match("Cxperpre", names(newdata))], nsim = 500)
  #Doing the predictions
  pre <- gjamPredict(output = model, newdata = newdata)
  #return the mean of the y-chains for every observation
  return(pre$sdList$yMu[,2])
...
set.seed(1980)
#DOing it for 10 permutations
vi_at_con <- model_parts(dal_at_con, type = "difference", B = 5)
