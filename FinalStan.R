...
#5. Redoing the internal validation for the "most appropriate" model.#1. Data preparation and fitting of final model
#6. Results
...
rhs_formula <- ~ Month + (IA_500 + NDVI_500 + IABEF_2000 + NDVIBEF_2000)^2 +
...
fit_com_cp <- stan_glm(full_formula_cp, data = train, refresh = 0, family = binomial(link = "probit"), 
                   init_r = .7, seed = 333, iter = 2000)
#Maximum Rhat-value of all parameters
maxRhat_rs(fit_com_cp)
#Chains seem to converge.

#Anopheles atroparvus
full_formula_at <- as.formula(paste("Anatropre ~", as.character(rhs_formula)[2]))
fit_com_at <- stan_glm(full_formula_at, data = train, refresh = 0, family = binomial(link = "probit"), 
                   init_r = .7, seed = 333, iter = 2000)
#Maximum Rhat-value of all parameters
maxRhat_rs(fit_com_at)
#Chains seem to converge.
...
################################3. Internal Validation of the most complex model with DHARMa########
...
dharm_cp <- createDHARMa(simulatedResponse = t(posterior_predict(fit_com_cp)), observedResponse = fit_com_cp$y,
                         fittedPredictedResponse = posterior_predict(fit_com_cp) %>% apply(2, median), integerResponse = T, seed = 333,
                         method = "PIT")
plot(dharm_cp)
...
plotResiduals(dharm_cp, train$IABEF_2000)#significant problems (.25 and .75 quantiles) and also combined
#quantile test is significant
plotResiduals(dharm_cp, train$NDVIBEF_2000)#no problems
plotResiduals(dharm_cp, train$Month) #looks good
#the quadratic forms
...
plotResiduals(dharm_cp, train$NDVI_500*train$NDVIBEF_2000) # .25 + combined quantile test is significant
plotResiduals(dharm_cp, train$IABEF_2000*train$NDVIBEF_2000)# no problems
#Overall, only two plots (and 3 tests) had significant problems. Due to the high number of plots,
#this is somewhat  expected (we performed 15*3 = 45 significance tests >> 
#hence we would expect roughly 2 significant tests (95%-level))
...
dharm_at <- createDHARMa(simulatedResponse = t(posterior_predict(fit_com_at)),
                         observedResponse = fit_com_at$y,
                         fittedPredictedResponse = posterior_predict(fit_com_at) %>% apply(2, median),
...
plotResiduals(dharm_at, train$IABEF_2000)#significant problems(.25 quantile + combined quantiles)
plotResiduals(dharm_at, train$NDVIBEF_2000)#.25 quantile is significant + combined quantile
#test also significant
plotResiduals(dharm_at, train$Month) #looks good
#the quadratic forms
plotResiduals(dharm_at, (train$IA_500)^2) # .25 quantile + combined quantile test are significant
...
#We just look at the unconditional predictions of both species at once
#calculating the in-sample predictions (simulations)
sim <- predict_mul(fit_com_mul, rhs_formula, train, n_max = 4000) 

#' The single draws from the predictive posterior for the unconditional predictions are stored in
#'  $p1_uncond and $p2_uncond. We combine both in one matrix:
sim <- rbind(sim$p1_uncond, sim$p2_uncond)
#'    The predictions are on the probability scale. In order to obtain simulated y-values on the 
#'observational scale that include the residual error, we must draw from a binomial distribution with
#' the probility that we drew from the predictive posterior distribution (rbinom does the job for us)
sims <- matrix(rbinom(n = length(sim),size = 1, sim),
               nrow = nrow(sim)) 

#creating the DHARMa object
dharm_mv_un <- createDHARMa(simulatedResponse = sims,
                            observedResponse = append(fit_com_cp$y, fit_com_at$y, after = length(fit_com_cp$y)),
                            fittedPredictedResponse = rowMedians(sim),
                            integerResponse = T, seed = 333,method = "PIT")
...
#2. KS-test also supports our assumption that the scaled residuals are uniformly 
#distributed >> our model is correctly specified.
#3. Residual vs. predicted quantile deviations (insignificant): The scaled residuals don't increase
#with the higher predictions on the probability scale.

#Plotting the residuals against all covariates to check whether we specified the
# functional relationships correctly.
plotResiduals(dharm_mv_un, rep(train$IA_500,2)) #looks ok, no significant problems
plotResiduals(dharm_mv_un, rep(train$NDVI_500, 2))#looks ok, no significant problems
plotResiduals(dharm_mv_un, rep(train$IABEF_2000, 2))#no significant problems
plotResiduals(dharm_mv_un, rep(train$NDVIBEF_2000, 2))#significant problems for all
#quantiles and the combined test
plotResiduals(dharm_mv_un, rep(train$Month, 2)) #looks good
#the quadratic forms
plotResiduals(dharm_mv_un, rep((train$IA_500)^2, 2)) # looks ok
plotResiduals(dharm_mv_un, rep((train$NDVI_500)^2, 2)) # looks ok
plotResiduals(dharm_mv_un, rep((train$IABEF_2000)^2, 2)) # looks ok
plotResiduals(dharm_mv_un, rep((train$NDVIBEF_2000)^2, 2)) # looks ok
#the interactions
plotResiduals(dharm_mv_un, rep(train$IA_500*train$NDVI_500, 2))#looks ok
plotResiduals(dharm_mv_un, rep(train$IA_500*train$IABEF_2000, 2)) # looks ok
plotResiduals(dharm_mv_un, rep(train$IA_500*train$NDVIBEF_2000, 2)) # looks ok
plotResiduals(dharm_mv_un, rep(train$NDVI_500*train$IABEF_2000, 2)) # looks ok
plotResiduals(dharm_mv_un, rep(train$NDVI_500*train$NDVIBEF_2000, 2)) # looks ok
plotResiduals(dharm_mv_un, rep(train$IABEF_2000*train$NDVIBEF_2000, 2))# no problems
#Overall, no plot had significant problems >> So, model seems to be correctly specified

hist(dharm_mv_un)
#looks pretty flat >> thumbs up!

##Test for temporal autocorrelation
dharm_mv_un_auto = recalculateResiduals(dharm_mv_un, group = rep(train$Fecha, 2))
testTemporalAutocorrelation(dharm_mv_un_auto, time =  unique(train$Fecha))
...
dharm_mv_un_spatial <- recalculateResiduals(dharm_mv_un,
                                            group = rep(train$Norte, 2))
testSpatialAutocorrelation(dharm_mv_un_spatial, 
                           x =  aggregate(train$Oeste, list(train$trap), mean)$x, 
                           y = aggregate(train$Norte, list(train$trap), mean)$x)
...
z <- c("IA_500", "NDVI_500", "IABEF_2000", "NDVIBEF_2000", "Month")

step_waic_entCov(fit_com_cp, z)
...
#interquad function):
fitr_cp <-  stan_glm(Cxperpre ~ (IA_500 + NDVIBEF_2000)^2 + Month +
...
#Running the step-function that drops interactions and quadratic specifications
interquad(fitr_cp, z)
...
#of WAIC:
fitp_cp <- stan_glm(Cxperpre ~ Month + IA_500 + NDVIBEF_2000 +
                      I(IA_500^2) + I(NDVIBEF_2000^2), data = train,
...
#running the step-function for dropping covariates and all their associations
step_waic_entCov(fit_com_at, z)
#The function dropped two variables NDVI_500 and IABEF_2000. So, the new model
#includes IA_500, NDVIBEF_2000 and the interactions as well as the quadratic terms
#and the factor Month. (Same model as for CP)

#Running this model and storing it (we need it as an input for the
#interquad function):
fitr_at <-  stan_glm(Anatropre ~ (IA_500 + NDVIBEF_2000)^2 + Month +
                       I(IA_500^2) + I(NDVIBEF_2000^2), data = train,
                     refresh = 0, family = binomial(link = "probit"), 
                     init_r = .7, seed = 333)

#Dropping of interactions and quadratic specifications
#defining a names vector
z <- c("IA_500", "NDVIBEF_2000")
interquad(fitr_at, z)
#So, we drop the only remaining interaction term and the quadratic
#term of IA_500. 
fitp_at <- stan_glm(Anatropre ~ IA_500 + NDVIBEF_2000 + Month +
...
fitcp_at <- stan_glm(Anatropre ~ Month + IA_500 + NDVIBEF_2000 +
                       I(IA_500^2) +  I(NDVIBEF_2000^2), data = train,
                     refresh = 0, family = binomial(link = "probit"), 
                     init_r = 1.4, seed = 333)
...
#for CP
fitat_cp <- stan_glm(Cxperpre ~ IA_500 + NDVIBEF_2000 + Month +
                       I(NDVIBEF_2000^2), data = train,
                     refresh = 0, family = binomial(link = "probit"), 
                     init_r = .7, seed = 333)
waic(fitat_cp)
...
#of the models and 2. the difference in WAIC between the two models is smaller
#for AT.
...
#fitting the according multivariate model
#' Model formula - only the right hand side (i.e. the side for the predictors)
...
#For cp
dharm_fin_cp <- createDHARMa(simulatedResponse = t(posterior_predict(fit_cp)), observedResponse = fit_cp$y,
...
plotResiduals(dharm_fin_cp, train$Month) #looks good
#the quadratic forms
...
#For at
dharm_fin_at <- createDHARMa(simulatedResponse = t(posterior_predict(fit_at)), observedResponse = fit_at$y,
...
#Plotting the residuals against all covariates to check whether we specified the
# functional relationships correctly.
plotResiduals(dharm_fin_at, train$IA_500) #.25 quantile and combined quantile test are significant
plotResiduals(dharm_fin_at, train$NDVIBEF_2000)#.25 quantile is significant
#and combined quantile test also significant
plotResiduals(dharm_fin_at, train$Month) #looks good
#the quadratic forms
...
#Overall, DHARMa flags issues with the model: For many covariates, the residuals are dependent
#on the values of the covariates. This is an indication for misspecifications in
#the model (omitted variable bias or functional misspecification)

####For multivariate model####
####For the multivariate model: We just look at the unconditional predictions of both species at once
#calculating the in-sample predictions (simulations)
sim_fin <- predict_mul(fit_mul, rhs_formula, train, n_max = 4000) 

#' The single draws from the predictive posterior for the unconditional predictions are stored in
#'  $p1_uncond and $p2_uncond. We combine both in one matrix:
sim_fin <- rbind(sim_fin$p1_uncond, sim_fin$p2_uncond)
#'    The predictions are on the probability scale. In order to obtain simulated y-values on the 
#'observational scale that include the residual error, we must draw from a binomial distribution with
#' the probility that we drew from the predictive posterior distribution (rbinom does the job for us)
sims_fin <- matrix(rbinom(n = length(sim),size = 1, sim),
               nrow = nrow(sim)) 

#creating the DHARMa object
dharm_mv_un_fin <- createDHARMa(simulatedResponse = sims_fin,
                            observedResponse = append(fit_cp$y, fit_at$y, after = length(fit_cp$y)),
                            fittedPredictedResponse = rowMedians(sim_fin),
                            integerResponse = T, seed = 333,method = "PIT")

plot(dharm_mv_un_fin)
...
#Plotting the residuals against all covariates to check whether we specified the
# functional relationships correctly.
plotResiduals(dharm_mv_un_fin, rep(train$IA_500,2)) #looks ok, no significant problems
plotResiduals(dharm_mv_un_fin, rep(train$NDVIBEF_2000, 2))#looks ok, no significant problems
plotResiduals(dharm_mv_un_fin, rep(train$Month, 2)) #looks good
#the quadratic forms
plotResiduals(dharm_mv_un_fin, rep((train$IA_500)^2, 2)) # looks ok
plotResiduals(dharm_mv_un_fin, rep((train$NDVIBEF_2000)^2, 2)) # looks ok

#Overall, no significant problems

hist(dharm_mv_un_fin)
#looks pretty flat >> thumbs up!

##Test for temporal autocorrelation
dharm_mv_un_fin_auto = recalculateResiduals(dharm_mv_un_fin, group = rep(train$Fecha, 2))
testTemporalAutocorrelation(dharm_mv_un_fin_auto, time =  unique(train$Fecha))
#doesn't  seem to be a problem

#Spatial Autocorrelation
dharm_mv_un_fin_spatial <- recalculateResiduals(dharm_mv_un_fin,
                                                group = rep(train$Norte, 2))
testSpatialAutocorrelation(dharm_mv_un_fin_spatial, 
                           x =  aggregate(train$Oeste, list(train$trap), mean)$x, 
                           y = aggregate(train$Norte, list(train$trap), mean)$x)
#No spatial autocorrelation >> Yeah!
...
################################6.1.a. Comparing the coefficients: Size and credibility intervals####
